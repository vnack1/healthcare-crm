        ...
        <section class="section">
            <div class="row" id="basic-table">
                <div class="col-12 col-md-12">
                        ...
                        <div class="card-content">
                            ...
                            <div class="table-responsive m-detail-table">
                                <table class="table mb-0 table-lg" id="table3">
                                    ...
                                    <?php
                                        // 현재 URL에서 ref 파라미터 읽기
                                        $referrer = $_GET['ref'] ?? 'all'; // ref가 없으면 기본값을 'all'로 설정
                                    ?>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="<?= isset($inputAllowed[$week]) && $inputAllowed[$week] ? 'clickable' : 'disabled' ?>"
                                                ...
                                                onclick="location.href='/melosira-management/melosira/enroll?user_idx=<?= esc($user['user_idx']) ?>&week=<?= $week ?>&ref=<?= $_GET['ref'] ?? 'all' ?>'"
                                                style="cursor: pointer; background-color: #caf4c7;" <?php else: ?>
                    ...
                    <div class="fs-6 mt-3 ms-4">** 각 주차별 정보를 입력하시려면 <span class="text-danger fw-bolder">초록색</span>으로 
                        표시되는 <span class="text-danger fw-bolder">"등록 가능" 박스를 클릭</span>하세요 (등록 가능한 주차만 선택 가능합니다)</div>
        </section>
        <!-- 버튼들 -->
        <div class="d-flex justify-content-center gap-3 pt-4 pb-3 col-12">
            <?php
            $referrer = $_GET['ref'] ?? 'all'; // ref 기본값을 'all'로 설정
        ?>
            <a class="btn btn-secondary d-flex align-items-center justify-content-center"
                style="width: 160px; font-size: 18px"
                href="<?= $referrer === 'complete' ? base_url('melosira-management/complete_list') : base_url('melosira-management/all_list') ?>">
                목록으로</a>
        </div>
    </div>
